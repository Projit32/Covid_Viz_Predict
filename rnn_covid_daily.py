...
mc = ModelCheckpoint('Models/best_model_daily_v1.h5', monitor='loss', mode='min', verbose=1, save_best_only=True)

regressor.fit(X_train, y_train, epochs = 50,  batch_size = 4, callbacks=[mc])

regressor=load_model('Models/best_model_daily_v1.h5')
...
plt.plot(y_test_unscaled, color = 'red', label = 'Real Daily cases')
...
regressor.fit(X_test, y_test, epochs = 50,  batch_size = 2, callbacks=[new_mc])
...
plt.plot(y_test_unscaled, color = 'red', label = 'Real Daily cases')
plt.plot(y_pred_new, color = 'green', label = 'Predicted Daily cases')
plt.title('Daily Cases Trend Testing After Training')
plt.xticks(list(range(0,len(y_pred))), test_dates, rotation='vertical')
plt.xlabel('Date')
plt.ylabel('No. of Cases')
plt.legend()
